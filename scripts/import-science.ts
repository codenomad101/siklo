...
async function main() {
  ...
  try {
    const cats = await sql/*sql*/`select category_id, name, slug from practice_categories where slug = 'science' or lower(name) LIKE '%science%' limit 1` as any[];
    
    if (!cats || cats.length === 0) {
      console.log('Science category not found. Available categories:');
      const allCats = await sql/*sql*/`select category_id, name, slug from practice_categories limit 20` as any[];
      console.log(JSON.stringify(allCats, null, 2));
      throw new Error("Science category not found. Ensure slug is 'science' or name contains 'Science'.");
    }
    
    console.log('Found Science category:', cats[0]);
    const categoryId = cats[0].category_id as string;

    const candidates = [
      path.resolve(process.cwd(), 'data/English/science/scienceEnglish.json'),
      path.resolve(process.cwd(), 'data/English/scienceEnglish.json'),
      path.resolve(process.cwd(), 'data/scienceEnglish.json'),
    ];
    const filePath = candidates.find((p) => fs.existsSync(p));
    if (!filePath) throw new Error('scienceEnglish.json not found.');
      ...
      try {
        ...
        await sql/*sql*/`
          insert into practice_questions (
            ...
            ${q.category || 'Science'}, ${path.basename(filePath)}, 'active', ${topicRaw || null}
          )
        `;
        inserted++;
      } catch (e) {
    ...
    console.log(`Imported Science questions. inserted=${inserted}, skipped=${skipped}, failed=${failed}. category total now=${count}`);
  } finally {
    // @ts-ignore
