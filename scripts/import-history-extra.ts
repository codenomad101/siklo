...
async function main() {
  ...
  try {
    const cats = await sql/*sql*/`select category_id from practice_categories where slug = 'history' limit 1` as any[];
    if (!cats || cats.length === 0) throw new Error('History category not found.');
    ...
    if (!filePath) throw new Error('historyExtra.json not found.');
